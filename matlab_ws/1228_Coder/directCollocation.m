function soln = directCollocation(problem) %trapezoidしてる
...
[zGuess, pack] = packDecVar(guess.time, guess.state, guess.control); %離散化
if flagGradCst || flagGradObj %or
    gradInfo = grad_computeInfo(pack); %勾配の変換
end

% Unpack all bounds:　　　tLow<t<tUpp
...
%%%% Set up problem for fmincon: ??なにしてんの
if flagGradObj
    P.objective = @(z)( ...
...
%
% OUTPUTS:
%   cost = scale cost for this set of decision variables  %??scale
%   cost評価関数かな
%
