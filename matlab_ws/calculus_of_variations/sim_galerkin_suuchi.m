...
in_int_mf=matlabFunction(in_int) % Symbolic変数からMATLABデータに変換
in_int_mf2=@(x_R)in_int_mf(x_R,alpha1,alpha2,alpha3) % 変数をさらにx_Rに絞った場合（alpha1~3は放置）

% L=integral(in_int_mf,r_R0(1),r_H0(1),"ArrayValued",true) % エラー：入力引数が不足しています（x_R,alpha1~3の4変数関数として積分しようとしている）

L=integral(in_int_mf2,r_R0(1),r_H0(1),"ArrayValued",true) % エラー：関数または変数 'alpha1' が認識されません。（1変数関数としてみなしているが、変数自体には値が入っていないから数値積分できない？）
