function result=MAIN_func()
    ...
    for candidate4=[-0.6 -0.7 -0.8 -0.9 -1.0 -1.1 -1.2]
        for candidate3=[0.10 0.15 0.20 0.25 0.30]
            for candidate=[7.5 10]
                for candidate2=[-3 -3.5 -4 -4.5 -5]
                    try
                        date="1219";
                        abst="realsense_parameter_study_hmn_vx";
                        detail="x"+string(candidate)+"_y"+string(abs(candidate2))+"_slc"+string(candidate3);
                        mkdir('results');
                        ...
                        env.hz=abs(hmn.vx)*40/3; % 0.6m/s -> 8Hz
